...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const data = await fetchJson("/api/post/getposts");
        setPosts(data.posts || []);
      } catch (fetchError) {
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-mist via-white to-subtle dark:from-slate-900 dark:via-slate-950 dark:to-slate-900">
        <PageContainer className="flex flex-col gap-10 pb-12 pt-12 lg:flex-row lg:items-end lg:pb-16">
          <div className="flex-1 space-y-4">
            ...
            <p className="max-w-2xl text-lg text-slate-600 dark:text-slate-200">{heroDescription}</p>
          ...
          {featuredPost && (
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-3 p-6">
                ...
                <h2 className="text-xl font-bold leading-tight text-primary dark:text-white">{featuredPost.title}</h2>
                <p className="text-sm text-slate-600 line-clamp-3 dark:text-slate-300">{heroDescription}</p>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            {!loading &&
              ...
              latestPosts.map((post) => (
                ...
                >
                  <Link href={`/post/${post.slug}`} className="flex items-start gap-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <h3 className="text-base font-semibold leading-snug text-slate-900 line-clamp-2 dark:text-white">{post.title}</h3>
